...
namespace Teste_Tecnico_Tapps
{
    class CautiousPlayer : Player
    {
        public override bool BuyOrPay(Propriedade prop)
        {
            ...
            if (prop.Owner == null)
            {
                // Checking if has enough Coins & if satisfies the Cautiousness of the player
                if (prop.GetValue() <= this.PlayerCoins && this.PlayerCoins - prop.GetValue() >= 80)
                {
            ...
            else
            {
                ...
                {
                    // Optional Output - Console.WriteLine($"\n       O Jogador do tipo Cauteloso foi Eliminado pois ficaria com {this.PlayerCoins - prop.GetRent()} de saldo para pagar o aluguel");
